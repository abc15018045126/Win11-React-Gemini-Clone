...
const Chrome5App: React.FC<AppComponentProps> = ({ setTitle: setWindowTitle }) => {
    ...
    const partition = `persist:chrome5`;
    ...
    useEffect(() => {
        ...
        const handleLoadStop = () => {
            ...
            if (currentUrl && !currentUrl.startsWith('about:blank')) {
                setWindowTitle(`${webview.getTitle()} - Chrome 5`);
                setInputValue(currentUrl);
            } else {
                setWindowTitle(`New Tab - Chrome 5`);
            }
            setCanGoBack(webview.canGoBack());
            setCanGoForward(webview.canGoForward());
        };
        const handleCrash = () => {
            console.error("!!! CHROME 5 WEBVIEW CRASHED !!!");
            setWindowTitle("Browser Crashed - Chrome 5");
            setIsLoading(false);
        };
